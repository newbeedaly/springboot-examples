...
public class Recver1 {
  ...
  public static void main(String[] args) throws Exception{
    ...
    channel.queueDeclare("testroute1",false,false,false ,null );
    // 绑定队列到交换机
    // 参数3 标记，绑定到交换机的时候指定标记，只有和他一样的标记才会收到消息
    channel.queueBind("testroute1",EXCHANGE_NAME,"key1");
    // 接收多个标记，再复制一次
    channel.queueBind("testroute1",EXCHANGE_NAME,"key2");
    channel.basicQos(1);
    // 接收消息
    final DefaultConsumer defaultConsumer = new DefaultConsumer(channel){
      ...
      public void handleDelivery(String consumerTag, Envelope envelope, AMQP.BasicProperties properties, byte[] body) throws IOException {
        // 当我们收到消息
        System.out.println("路由消费者1收到的内容是："+new String(body));
        channel.basicAck(envelope.getDeliveryTag(),false);
      }
    };
    channel.basicConsume("testroute1",false,defaultConsumer);
  }
}
